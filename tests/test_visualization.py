...
def test_plot_cluster_vote_distribution():
    ...
    # Create sample data
    clusters = {0: ["US", "UK", "CA"], 1: ["FR", "DE", "IT"]}
...
def test_plot_resolution_polarity():
    ...
    # Create sample data
    df_filtered = pd.DataFrame(
        {"issue": ["Test Issue"] * 6, "vote": ["Y", "N", "A", "Y", "N", "A"]}
    )
...
def test_plot_entropy_distribution():
    ...
    entropy_scores = pd.Series(
        {"US": 0.8, "UK": 0.7, "FR": 0.6, "DE": 0.5, "IT": 0.4, "CA": 0.3}
    )
...
def test_edge_cases():
    ...
    assert (
        plot_cluster_vote_distribution(empty_clusters, empty_df) is None
    )  # Changed expectation
