...
def test_load_and_preprocess_data():
    ...
    invalid_data = pd.DataFrame(
        {"random_col": [1, 2, 3], "date": ["2020-01-01", "2020-01-02", "2020-01-03"]}
    )
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".csv", delete=False
    ) as temp_file:
...
def test_create_vote_matrix():
    ...
    sample_data = pd.DataFrame(
        {
            ...
            "rcid": [1, 2, 1, 2, 1, 2],
            "vote": [1, -1, 1, 0, -1, 1],  # Using mapped numeric values
            "date": pd.date_range(start="2020-01-01", periods=6),
            "year": [2020, 2020, 2020, 2020, 2020, 2020],
        }
    )

    vote_matrix, country_list, df_filtered = create_vote_matrix(sample_data, 2020, 2020)
    ...
    vote_matrix, country_list, df_filtered = create_vote_matrix(sample_data, 2019, 2021)
    assert vote_matrix is not None
    assert len(country_list) == 3

    # Test with empty year range
    vote_matrix, country_list, df_filtered = create_vote_matrix(sample_data, 2021, 2022)
...
def test_calculate_country_entropy():
    ...
    single_vote_df = pd.DataFrame(
        {"country_identifier": ["US", "UK"], "vote": ["Y", "N"]}
    )
    entropy_scores = calculate_country_entropy(single_vote_df)
    assert len(entropy_scores) == 2
    assert all(
        score == 0 for score in entropy_scores
    )  # Single vote should have 0 entropy
    ...
    assert all(
        score == 0 for score in entropy_scores
    )  # All same votes should have 0 entropy
    ...
    valid_votes_df = pd.DataFrame(
        {
            ...
            "rcid": [1, 1],
            "vote": [1, -1],  # Using numeric values
            "date": pd.date_range(start="2020-01-01", periods=2),
            "year": [2020, 2020],
        }
    )
    vote_matrix, country_list, df_filtered = create_vote_matrix(
        valid_votes_df, 2020, 2020
    )
    assert isinstance(vote_matrix, pd.DataFrame)
    ...
    invalid_votes_df = pd.DataFrame(
        {
            "country_identifier": ["US", "UK"],
            "rcid": [1, 1],
            "vote": ["Invalid", 1],  # Invalid vote value (will become NaN -> 0)
            "date": pd.date_range(start="2020-01-01", periods=2),
            "year": [2020, 2020],
        }
    )
    vote_matrix, country_list, df_filtered = create_vote_matrix(
        invalid_votes_df, 2020, 2020
    )
    assert vote_matrix is not None
    assert (
        len(country_list) == 1
    )  # Only UK remains, US row with Invalid drops during pivot
