...
import pandas as pd
import numpy as np
from src.model import train_vote_predictor, predict_votes, save_model, load_model
import tempfile
import os

# Mark every test in this module as slow — they invoke logistic regression on
# DataFrames that, when combined with the global data load via test_app
# fixtures, can saturate CPU/memory on the full 911k-row dataset. Skip with:
#   pytest -m "not slow"
...
def test_train_vote_predictor():
    ...
    assert hasattr(model, "feature_context_")
    assert hasattr(model, "feature_columns_")
    assert "bloc" in model.feature_columns_
    assert "issue_salience" in model.feature_columns_
...
def test_predict_votes():
    ...
    # Test prediction
    vote_counts, detailed_predictions = predict_votes(
        model,
        countries,
        "Issue 1",
...
def test_model_persistence():
    ...
    with tempfile.NamedTemporaryFile(suffix=".joblib") as tmp:
        ...
        vote_counts, detailed_predictions = predict_votes(
            loaded_model, countries, "Issue 1"
        )
        ...
        original_vote_counts, original_detailed_predictions = predict_votes(
            model, countries, "Issue 1"
        )
