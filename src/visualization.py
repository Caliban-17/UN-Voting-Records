...
def plot_cluster_vote_distribution(
    clusters: Dict[int, List[str]], df_filtered: pd.DataFrame, n_cols: int = 4
) -> Optional[plt.Figure]:
    ...
    try:
        ...
        for i, (cluster_id, countries) in enumerate(sorted(clusters.items())):
            cluster_votes = df_filtered[
                df_filtered["country_identifier"].isin(countries)
            ]["vote"]
            vote_counts = cluster_votes.value_counts(normalize=True).reindex(
                ["Y", "N", "A", " "], fill_value=0
            )
...
def plot_pca_projection(
    vote_matrix: pd.DataFrame, cluster_labels: np.ndarray, n_clusters: int
) -> Optional[plt.Figure]:
    ...
    try:
        ...
        pca_df = pd.DataFrame(
            data=pca_result, columns=["PC1", "PC2"], index=vote_matrix.index
        )
...
def plot_issue_salience(
    df_filtered: pd.DataFrame, n_top_issues: int = 10
) -> Optional[plt.Figure]:
    """Plots timeline of issue salience."""
    try:
        issue_counts = (
            df_filtered.groupby(["year", "issue"])["rcid"]
            .nunique()
            .reset_index(name="vote_count")
        )
        top_issues = (
            issue_counts.groupby("issue")["vote_count"]
            .sum()
            .nlargest(n_top_issues)
            .index.tolist()
        )
        top_issue_counts = issue_counts[issue_counts["issue"].isin(top_issues)]
        issue_timeline = top_issue_counts.pivot(
            index="year", columns="issue", values="vote_count"
        ).fillna(0)
...
def plot_resolution_polarity(
    df_filtered: pd.DataFrame, issue: str
) -> Optional[plt.Figure]:
...
def plot_entropy_distribution(
    entropy_scores: pd.Series, top_n: int = 10
) -> Optional[plt.Figure]:
